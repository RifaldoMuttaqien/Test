...
class Exam_model extends CI_Model
{
    ...
    {
        return $this->db->where('exam_id', $id)->get('questions')->result();
    }
    ...
    public function get_my_results($id)
    {
        $result = $this->db->select('*')
                ...
                ->where('exam_title.student_can_see_result', 1)
                ->order_by("result.exam_taken_date", "desc")
                ->join('users', 'users.user_id = result.user_id', 'left')
                ->join('exam_title', 'exam_title.title_id = result.exam_id', 'left')
                ->get()
   ...
   public function evaluate_result()
    {
        ...
        if ($exam_detail->random_ques_no != NULL && $exam_detail->random_ques_no > 0)
            ...
            $total_ques = $this->db->where('exam_id', $exam_id)->count_all('questions');
